...
const VISUAL_DIMENSIONS=['sceneCount','averageSceneDuration','realFootageRatio','generatedImageRatio','generatedImageCount','diagramCount','scienceCardCount','zoomCount','replayCount','highlightCount','calloutCount','hookVisualType','captionDensity','visualStyle'];
function visualDimension(v,key){
 const m=v.mediaManifest;
 if(!VISUAL_DIMENSIONS.includes(key))return v[key];
 if(!m||m.preview||m.visualVersion!==1)return undefined;
 const x=m[key];if(x===undefined)return undefined;
 if(key==='realFootageRatio'||key==='generatedImageRatio')return x<.25?'low':x<.6?'balanced':'high';
 if(key==='averageSceneDuration')return x<3?'under3':x<=4?'3to4':'over4';
 if(key==='sceneCount')return x<8?'6to7':x<12?'8to11':'12plus';
 if(key==='captionDensity')return x<6?'low':x<9?'medium':'high';
 if(/Count$/.test(key))return x===0?'none':x<=2?'1to2':'3plus';
 return x;
}
function visualObservations(sample){
 if(sample.length<20)return [];
 const result=[];
 for(const dimension of VISUAL_DIMENSIONS){
  const groups=new Map();for(const x of sample){const k=visualDimension(x.v,dimension);if(k===undefined||x.m.origin!=='youtube')continue;const a=groups.get(k)||[];a.push(x.m);groups.set(k,a);}
  const eligible=[...groups].filter(([,a])=>a.length>=8);if(eligible.length<2)continue;
  for(const [value,ms] of eligible){const av=ms.map(m=>m.averageViewPercentage).filter(Number.isFinite);if(av.length<8)continue;result.push({dimension,value,n:ms.length,averageViewPercentage:mean(av),likes:mean(ms.map(m=>rate(m.likes,m.engagedViews)).filter(Number.isFinite)),comments:mean(ms.map(m=>rate(m.comments,m.engagedViews)).filter(Number.isFinite)),shares:mean(ms.map(m=>rate(m.shares,m.engagedViews)).filter(Number.isFinite)),subscribers:mean(ms.map(m=>rate(m.subscribersGained,m.engagedViews)).filter(Number.isFinite)),isCausalProof:false,note:'成熟した7集計日の観察比較。テーマや投稿時刻の影響を含み、因果関係は未確定。'});}
 }
 return result;
}
...
export function enrichStrategy(data,settings,isDemo=false){
 ...
 const allocations=[];
 for(const dimension of ['contentType','genre','captionStyle','narrationSpeed','editingStyle','weekday','sceneSeconds','visualStyle','hookVisualType','sceneCount','averageSceneDuration','realFootageRatio','generatedImageRatio','generatedImageCount','diagramCount','scienceCardCount','zoomCount','replayCount','highlightCount','calloutCount','captionDensity']){
  const groups=new Map();for(const x of sample){const k=String(visualDimension(x.v,dimension)??'未記録');if(k==='未記録')continue;if(!groups.has(k))groups.set(k,[]);groups.get(k).push(x.reward);}
  const visual=VISUAL_DIMENSIONS.includes(dimension)||dimension==='sceneSeconds';const eligible=[...groups].filter(([,a])=>a.length>=(visual?8:5));if(sample.length<(visual?20:12)||eligible.length<2)continue;
  ...
  for(const x of scored)allocations.push({...x,isCausalProof:false,share:Math.round((.7*Math.exp(x.score/20)/sum+.3/scored.length)*1000)/10});
 }
 data.memory.visualPatterns=visualObservations(sample);
...
export function chooseAllocation(memory,dimension,fallback,rng=Math.random){
 // Visual First now applies to A and B. Keep the existing 30% exploration budget.
 const values=(memory.allocations||[]).filter(a=>a.dimension===dimension);if(!values.length)return fallback;let r=rng()*values.reduce((s,x)=>s+x.share,0);for(const x of values){r-=x.share;if(r<=0)return x.value;}return fallback;
}
