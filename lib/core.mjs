...
import {musicPreferences,selectMusic,normalizeTrack} from './shorts-music.mjs';
...
export function digestable(v) { return JSON.stringify([v.title,v.description,v.privacy,v.madeForKids,v.containsSyntheticMedia,v.tags,v.publishAt,v.segments,v.videoHash,v.revision,v.assetId,v.assetIds,v.segmentAssets,v.scenePlan,v.mediaManifest,v.verifiedContentHash,...(v.musicChoice?[v.musicChoice]:[])]); }
export function applyAction(state,action,payload={},dataset='live') {
 ...
 if(action==='musicSettings'){assert(dataset==='live','実チャンネルの設定を選んでください。');state.settings.music=musicPreferences(payload);log(data,'music','希望曲と自動選曲の設定を保存しました。YouTubeでの音源追加は未完了です。');return;}
 if(action==='selectMusic'){
  const v=data.videos.find(x=>x.id===payload.id);assert(v&&!v.youtubeId&&!v.uploadIntent&&!v.uploadSession&&['draft','review','blocked','approved'].includes(v.status),'未送信の動画だけ選曲を変更できます。');
  v.requestedTrack=normalizeTrack(payload.track);v.musicChoice=selectMusic(v,state.settings.music);v.revision=(v.revision||0)+1;v.approvedRevision=null;delete v.approvedDigest;if(v.status==='approved')v.status='review';
  if(v.mediaManifest?.musicEvidence?.mode==='shorts-library')v.mediaManifest.musicEvidence.choice=v.musicChoice;
  log(data,'music',`「${v.musicChoice.track.title}」を希望曲として保存。音源はまだ追加されていません。`);return;
 }
 if(action==='settings'){
