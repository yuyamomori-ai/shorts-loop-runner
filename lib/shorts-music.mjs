...
export function selectMusic(v,preferences={},at=Date.now()){
 ...
 const candidate=forced||(!catalog.fresh?prefs.tracks[0]:intent==='説明を聞き取りやすく'?catalog.tracks[1]:catalog.tracks[0]);
 ...
 return {track:normalizeTrack(candidate),intent,selection:forced?'owner':'automatic-candidate',status:'pending_native_selection',
  reason:forced?'指定された希望曲です。':catalog.fresh?'日本のShorts公式ランキングを参考にした候補です。曲と内容の相性は編集上の推定です。':'ランキングの確認期限を過ぎたため希望曲を候補にしています。現在の人気は未確認です。',
