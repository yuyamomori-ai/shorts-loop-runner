...
export function requestPublicAutopilot(state,requestId) {
  ...
  const editable=['冒頭を1〜2秒で読める長さにできませんでした。','検索結果で確認できない情報源が含まれています。','TYPE Bには権利確認済みの素材、またはPEXELS_API_KEYが必要です。','予算見積もりとの差を検出しました。費用を確認するまで生成を保留します。'].includes(a.reason||'');
