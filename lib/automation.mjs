...
function perVideoRejection(reason){
 ...
 if(['図解は同じセグメントの出典で裏付けてください。','企画の出典IDが登録された資料にありません。','台本の形式が不正です。','生成タイトルが不正です。','AIから有効なJSONを取得できませんでした。','再編集した台本の形式・根拠IDが不正です。'].includes(text))return true;
...
export function requestPublicAutopilot(state,requestId,{repairVideoId}={}) {
  ...
  const malformed=['台本の形式が不正です。','生成タイトルが不正です。','AIから有効なJSONを取得できませんでした。','再編集した台本の形式・根拠IDが不正です。'].includes(a.reason||'');
