import {repairableVisualReview} from './visual.mjs';
import {presentationFailure} from './render-failures.mjs';
// Durable intent: rejected videos never become publishable without fresh QA.
...
function perVideoRejection(reason){
 ...
 if(presentationFailure(text)||text==='冒頭画像が暗いため投稿を保留します。')return true;
 if(['図解は同じセグメントの出典で裏付けてください。','企画の出典IDが登録された資料にありません。','台本の形式が不正です。','生成タイトルが不正です。','AIから有効なJSONを取得できませんでした。','再編集した台本の形式・根拠IDが不正です。','自動修正の台本が不正です。','再編集した台本が長すぎます。','読み上げ台本に図解記号やメモ書きが残っています。'].includes(text))return true;
 if(['実験・戦略条件と生成結果が一致しません。','取得可能な一次資料が足りないため、根拠のない企画は制作しません。','一次情報の数が不足しています。','解説・編集の付加価値が不足したため、この動画を見送ります。'].includes(text))return true;
...
export function requestPublicAutopilot(state,requestId,{repairVideoId}={}) {
  ...
  // An explicit owner retry may reuse a fact-checked, unsent visual-only rejection.
  // No fact/rights flags or upload session are cleared. Rendering verifies them again.
  const v=repairVideoId&&(state.live?.videos||[]).find(v=>v.id===repairVideoId);
  const repair=!!(v&&['blocked','draft'].includes(v.status)&&v.qa?.visual==='failed'&&!v.risk&&!v.youtubeId&&!v.uploadIntent&&!v.uploadSession&&v.qa?.facts==='passed'&&repairableVisualReview(v.visualQa));
  const productionStop=perVideoRejection(a.reason)||a.reason==='一次情報の数が不足しています。';
  if(repair&&(a.phase!=='attention'||productionStop)){
    Object.assign(v,{status:'draft',approvedRevision:null,initialPublicRequestId:requestId,plannedAt:new Date().toISOString(),publishAt:null,privacy:'public'});
    delete v.approvedDigest;delete v.error;v.qa.visual='pending';
  }
  const editable=['実験・戦略条件と生成結果が一致しません。','冒頭を1〜2秒で読める長さにできませんでした。','検索結果で確認できない情報源が含まれています。','情報源本文を確認できません。別の一次資料が必要です。','AIの出力が途中で終了しました。','事実確認が完了しなかったため、自動運転を停止しました。','TYPE Bには権利確認済みの素材、またはPEXELS_API_KEYが必要です。','予算見積もりとの差を検出しました。費用を確認するまで生成を保留します。'].includes(a.reason||'');
  const malformed=['台本の形式が不正です。','生成タイトルが不正です。','AIから有効なJSONを取得できませんでした。','再編集した台本の形式・根拠IDが不正です。','自動修正の台本が不正です。','再編集した台本が長すぎます。','読み上げ台本に図解記号やメモ書きが残っています。'].includes(a.reason||'');
  if(a.phase!=='attention'||billing||editable||malformed||repair&&productionStop){Object.assign(a,{userPaused:false,phase:'waiting',reason:null});state.settings.paused=true;delete a.retryAt;}
  return true;
}
...
export function pauseAutomation(state,reason,user=false) {
  ...
  if(!user&&perVideoRejection(text)){
    // A background preparation failure cannot undo an explicit pause or an
    // outstanding Google reauthorization. Keep the original blocking reason.
    if(state.automation?.userPaused||state.automation?.youtubeReconnectRequired){state.settings.paused=true;return;}
