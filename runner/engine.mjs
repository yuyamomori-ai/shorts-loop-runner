...
export class Engine {
 ...
 async generate(count,requestedType='auto',options={}){
  ...
  for(let i=0;i<count;i++){
   ...
   const v={...base,topic:String(x.topic||x.title).slice(0,120),title:x.title,genre:x.genre,hook:x.hook,structure:x.structure,segments:x.segments.map(y=>({text:y.text,role:['hook','body','answer','cta'].includes(y.role)?y.role:'body',sourceIds:y.sourceIds,...normalizeVisual(y)})),sources:x.sources.map(a=>({id:a.id,url:a.url,title:String(a.title).slice(0,300),publisher:String(a.publisher).slice(0,200),summary:String(a.summary).slice(0,600)})),duration:base.duration,durationBand:base.durationBand,hour:base.hour,privacy:s.settings.privacy,madeForKids:s.settings.madeForKids,description:'',qa:{facts:'pending',rights:'pending',technical:'pending',visual:'pending'},createdAt:now()};delete v.risk;
