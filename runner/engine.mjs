...
export class Engine {
 ...
 async reviewOriginality(v,result,asset){
  ...
  const q=await this.ai.response(`投稿前の独自性審査。事実の新発見や新しい研究データは要求せず、既知の事実をどう説明・編集したかの付加価値を評価する。reusedRiskは第三者またはチャンネルの既公開作品を付加価値なく再利用するリスクであり、同じ研究分野や一般的な科学知識を扱うだけで再利用と断定しない。比較履歴は実際に公開した動画だけで、未公開の失敗作・試作は含めていない。候補曲は未添付で、埋込済みの音源と取り違えない。音源追加待ちは別の公開ゲートで停止する。提示資料が足りなければ不足を明記し、権利・再利用リスクの基準を緩めない。自作台本と代表フレーム、実際の編集記録を審査。YouTubeの承認や収益化を保証しない。低い独自価値、直訳、テンプレート反復、実況だけ、著作権懸念、第三者の透かしは厳しく判定。TYPE Bでは、日本語の状況説明・独自解説・補足・オチが重要。事実と異なる煽りは不合格。JSON {"originality":0..100,"commentary":0..100,"editing":0..100,"educational":0..100,"entertainment":0..100,"copyrightRisk":0..100,"reusedRisk":0..100,"confidence":0..1,"reason":"日本語","fix":"改善案"}。企画=${JSON.stringify({type:v.contentType,title:v.title,segments:v.segments,methods:v.editingMethods})}。素材権利=${JSON.stringify(asset?{license:asset.license,evidence:asset.evidence,credit:asset.creditText,inspection:asset.inspection}:{license:'original'})}。編集=${JSON.stringify(result.manifest)}。出典と事実確認の記録=${JSON.stringify(evidence)}。出典タイトルとURLはワークスペースに保存されている。Shorts音源追加用マスターは未公開で、YouTube説明欄への反映もまだ完了していない。既公開動画=${JSON.stringify(published)}`,{images:samples.map(i=>result.frameFiles[i]).map(f=>'data:image/jpeg;base64,'+readFileSync(f).toString('base64'))});
 ...
 async generate(count,requestedType='auto',options={}){
  ...
  for(let i=0;i<count;i++){
   ...
   const v={...base,sourceEvidence:result.sourceEvidence,benchmarkTrial:normalizeBenchmarkTrial(x.benchmarkTrial,references),topic:String(x.topic||x.title).slice(0,120),title:x.title,genre:x.genre,hook:x.hook,structure:x.structure,segments:x.segments.map(y=>({text:y.text,role:['hook','body','answer','cta'].includes(y.role)?y.role:'body',sourceIds:y.sourceIds,...normalizeVisual(y)})),sources:x.sources.map(a=>({id:a.id,url:a.url,title:String(a.title).slice(0,300),publisher:String(a.publisher).slice(0,200),summary:String(a.summary).slice(0,600)})),duration:base.duration,durationBand:base.durationBand,hour:base.hour,privacy:s.settings.privacy,madeForKids:s.settings.madeForKids,description:'',qa:{facts:'pending',rights:'pending',technical:'pending',visual:'pending'},createdAt:now()};delete v.risk;
 ...
 async render(id,preview=false,repaired=false,lightweight=false){
  ...
  try{
   ...
   if(process.env.SHORTSLOOP_MUSIC_MODE==='shorts-library')this.store.update(s=>{const x=s.live.videos.find(x=>x.id===id);x.musicChoice=selectMusic(x,s.settings.music);});
