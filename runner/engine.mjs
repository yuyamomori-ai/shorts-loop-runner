...
export class Engine {
 ...
 async generate(count,requestedType='auto',options={}){
  ...
  for(let i=0;i<count;i++){
   ...
   if(x.risk==='none'&&!result.sourceEvidence){this.progress('source-plan-repair',base.id,{selected:(Array.isArray(x.sources)?x.sources:[]).map(s=>s?.url),observed:result.sources.slice(0,12)});result=await groundPlan(this.ai,result,{minSources:min,progress:(stage,extra)=>this.progress(stage,base.id,extra)});x=result.value;}
