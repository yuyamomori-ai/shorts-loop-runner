...
export function buildScenePlan(v,segments,assets=[],{repair=0}={}) {
 ...
 for(let i=0;i<segments.length;i++) {
  ...
  for(let j=0;j<count;j++) {
   ...
   scenes.push({index:scenes.length,segmentIndex:i,start,end,duration:end-start,visualType,assetId:card?null:assetId,effect,variant:(i+j+repair)%3,overlay:visual.overlay,callout:visual.callout,focus:visual.focus,sourceIds:s.sourceIds||[],diagramSpec:card?visual.diagramSpec:undefined,labels:fallbackLabels,sourceOffset:effect==='replay'?0:(i*2+j*1.2),transition:scenes.length?'cut':'opening',hook:i===0});
  }
 }
...
export function sceneFeatures(scenes,duration,captionChars=0) {
 ...
 const signature=s=>JSON.stringify([s.visualType,s.assetId,s.diagramSpec,s.labels,s.effect,s.variant]);
