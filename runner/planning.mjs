...
export async function groundPlan(ai,result,{minSources=1,preferredUrls=[],strategy=null,fetchSource=sourceText,progress=()=>{}}={}){
 ...
 return {...result,value:{...repaired.value,genre:strategy?.genre??result.value.genre,hook:strategy?.hook??result.value.hook,structure:strategy?.structure??result.value.structure,sources:registry},sources:registry.map(s=>s.url),sourceEvidence:evidence};
}
