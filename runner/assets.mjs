...
export async function discoverAsset(store,query){
 assert(process.env.PEXELS_API_KEY,'TYPE Bには権利確認済みの素材、またはPEXELS_API_KEYが必要です。');
 const response=await jsonFetch('https://api.pexels.com/videos/search?'+new URLSearchParams({query,per_page:'12',orientation:'portrait'}),{headers:{Authorization:process.env.PEXELS_API_KEY}});
 const state=store.read();const candidate=response.videos?.find(v=>v.duration>5&&v.duration<=120&&!state.live.assets.some(a=>a.providerId===v.id));assert(candidate,'未使用の利用可能な素材が見つかりません。');
 ...
 const file=choices[0];const r=await fetch(file.link,{redirect:'error',signal:AbortSignal.timeout(120000)});assert(r.ok,'素材の取得に失敗しました。');let n=0;const chunks=[];for await(const c of r.body){n+=c.length;assert(n<100*1024*1024,'素材サイズ上限を超えました。');chunks.push(c);}
 ...
 store.update(s=>{s.live.assets.push(a);});return attachAsset(store,a.id,Buffer.concat(chunks));
}
export async function inspectAsset(store,ai,asset){
 ...
 store.update(s=>{const a=s.live.assets.find(a=>a.id===asset.id);a.inspection=x;a.inspectedAt=now();a.containsPeople=!!x.containsPeople;if(!ok)a.rightsStatus='review';});assert(ok,'映像の権利・人物・状況に確信が持てないため、素材を確認待ちにしました。');return x;
}
