...
export function run(command,args,timeout=600000){if(command==='ffmpeg')args=['-threads',String(renderThreads),'-filter_threads',String(renderThreads),'-filter_complex_threads',String(renderThreads),...args];return new Promise((res,rej)=>{const child=spawn(command,args,{windowsHide:true});let out='',err='',diagnostics='';child.stdout.on('data',x=>out+=x);child.stderr.on('data',x=>{const chunk=x.toString();if(/black_start:|freeze_start:|Invalid data|Error while decoding/.test(chunk))diagnostics=(diagnostics+chunk).slice(-12000);err=(err+chunk).slice(-16000);});const t=setTimeout(()=>{child.kill();rej(new Error('処理が制限時間を超えました。'));},timeout);child.on('error',e=>{clearTimeout(t);rej(e);});child.on('close',code=>{clearTimeout(t);code===0?res({out,err:diagnostics+'\n'+err}):rej(new Error(`${command}: ${err.slice(-1200)}`));});});}
...
export async function validateMedia(file){
 ...
 const r=await run('ffmpeg',['-hide_banner','-v','info','-xerror','-i',file,'-vf','blackdetect=d=0.10:pix_th=0.07:pic_th=0.98,freezedetect=n=-50dB:d=5.8','-af','volumedetect','-f','null','-'],240000);
