...
// Reserve headroom after normalization and resampling; AAC can overshoot PCM peaks.
export const finalAudioFilter='loudnorm=I=-16:TP=-2:LRA=9,aresample=48000,alimiter=limit=0.794:level=0';
export async function validateMedia(file){
 ...
 assert(Number.isFinite(levels.peak)&&levels.peak<-.1&&levels.mean>-45,`音声の音量検査に失敗しました（peak=${levels.peak} dB / mean=${levels.mean} dB）。`);
