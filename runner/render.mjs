...
export async function renderVideo(v,{directory,ai,preview=false,lightweight=false,englishTest=false,asset=null,assets=[],repair=0,repairIssues=[],retainAudio=false}={}){
 ...
 const music=resolve(dir,'music.wav'),soundtrack=composeMusic(cursor,scenes.filter((s,i)=>!i||s.effect==='highlight'||s.callout).map(x=>x.start),v.id,v.musicStyle);writeFileSync(music,soundtrack.bytes);
