...
運用設定と動画詳細から希望曲を保存できます。自動候補は動画の内容と、
確認日を明示した日本の公式 Shorts 楽曲ランキングを参考にします。
`lib/shorts-music.mjs` のランキングは2026-09-13付の上位10曲の確認記録です。
一般のMV再生数とは区別し、7日を過ぎた順位は自動選曲に使いません。
日本の公式ランキングを確認する日次タスクを利用できます。実際に有効なタスクがある環境だけで
`SHORTSLOOP_CHART_REFRESH_SCHEDULED=true` を設定してください。取得失敗時は旧データを
新しい日付に書き換えません。順位は音源の利用権や配信可能性を証明しません。

`SHORTSLOOP_MUSIC_MODE=shorts-library` ではAI音声と自作効果音だけのマスターを作り、
候補曲・選定理由・参照した順位を保存します。YouTube公式APIにネイティブの
「サウンドを追加」を操作する公開機能がないため、曲の追加と公開はアプリで行います。
音源が未追加の動画を自動公開せず、設定画面にも待機理由を表示します。
設定や希望曲の変更は、送信済み動画に適用せず、未送信動画の承認を解除します。

PexelsのAPIキーは接続設定画面からも保存できます。既存のGoogle・OpenAI設定を保持し、
暗号化して保存します。最初の映像接続後、素材不足に伴う映像品質停止中の未送信制作を
再試行します。事実・権利に関する停止や明示的な一時停止は解除しません。
