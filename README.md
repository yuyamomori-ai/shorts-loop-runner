...
雑学・心理学・科学・歴史・記憶等を一次資料から企画。明るいAI生成画像（1本最大2枚）へのズーム・パン、根拠付き図解、AIナレーション、短い補助字幕、フリーBGMと効果音を組み合わせます。冒頭は大きな赤い見出しとイメージ画像を表示し、暗すぎる表紙は保留します。画像は説明用の概念イメージと明示し、科学的根拠や実験記録として扱いません。生成AIによる動画そのものの生成ではありません。

`OPENAI_IMAGE_MODEL=gpt-image-2` の1024×1536・mediumを固定し、最終映像は1080×1920。画像も既存の日次回数・月額AI枠に合算し、1枚$0.25を呼び出し前に予約、使用量が取得できれば精算します。不明な結果は予約額を維持します。台本・モデル・ファイルSHAが一致する画像は再編集時に再利用します。月1万円はAPI制作枠6,000円とホスティング等の留保4,000円の管理目標であり、外部サービス請求全体のハード上限を保証しません。

台本の形式だけが不正な場合は1回修正し、まだ不正ならその企画を見送って次へ進みます。事実・著作権・安全性の問題を修正済みとして扱いません。制作の試行回数、待機時間、予算の既存上限は継続します。
...
2026-09-25: 図解・イラストの背景を明るい配色に統一。冒頭画像が明るさ検査に不合格の場合は既存画像の露出を調整して、最大2回の映像修正枠内で再審査します。画像APIの再呼び出しはしません。投稿用の設定ミスで自作BGMへフォールバックすることも禁止しました（明示的な検証用素材・プレビューは別）。

音声生成後に尺・字幕の読速が不合格になった場合、台本の再編集を最大2回まで試し、その都度独立した事実確認をやり直します。合格しない動画は見送り、月額・日次のAI上限を保持します。復旧後に期限切れの予約を見つけた場合、自動運転の未送信・品質合格動画のみ次の空き枠へ移し、旧承認を解除して再承認します。手動承認、投稿済み、送信結果不明、権利・事実の問題はこの処理で解除しません。

起動時と15分ごとの `ShortLOOP status` ログで、制作済み・公開確認済み・Google再認可待ちを区別します。秘密値や送信セッションは記録しません。Google接続の失効は本人による再認可が必要で、設定だけで「公開成功」と扱いません。視聴実績に基づく派生分析の追加承認フラグも変更していません。

通常は `SHORTSLOOP_MUSIC_MODE=licensed` でフリーBGMを自動ミックスします。初期曲は Kevin MacLeod の **Monkeys Spinning Monkeys**（CC BY 4.0、公式配布・条件を2026-09-24に確認）。公式配布元から一度取得し、固定SHA-256と照合して非公開の永続キャッシュに保存。作者・曲名・出典・ライセンスURL・改変内容を毎回YouTube説明欄へ記載します。整合性や権利記録が欠けたら投稿を保留し、自作曲へ黙って置換しません。音源ファイルはGitへ含めません。しゃろう氏の音源は未登録です。自動取得禁止や個別の利用同意を回避して収集しません。

以下のネイティブ選曲候補は別機能です。フリーBGMの添付を、流行曲のネイティブ音源追加やランキング更新とは報告しません。
